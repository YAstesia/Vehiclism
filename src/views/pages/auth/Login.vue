...
const handleLogin = async () => {
    try {
        ...
        if (isSuccess) {
            ...
            localStorage.setItem('trim1', '');
            localStorage.setItem('trim2', '');
            localStorage.setItem('trim3', '');
            localStorage.setItem('trim4', '');
            localStorage.setItem('trim5', '');
            ...
            <div
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">欢迎来到Vehiclism！</div>
                        <span class="text-muted-color font-medium">登录以继续……</span>
                    </div>

                    <div>
                        <label for="id1"
                            class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">邮箱或手机号</label>
                        <InputText id="id1" type="text" placeholder="在此处输入邮箱或手机号" class="w-full md:w-[30rem] mb-8"
                            v-model="email" />

                        <label for="password1"
                            class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">密码</label>
                        <Password id="password1" v-model="password" placeholder="在此处输入密码" :toggleMask="true"
                        ...
                        <button @click="goBack"
                            class="w-full mt-4 bg-red-500 text-white py-2 px-4 rounded-md hover:bg-red-600"
                            style="height:35px">返回</button>
